...
class GC():
    ...
    def search(self):
        ...
        if person_doc == False:
            ...
            self.logger.debug(f"Person \"{value}\" rejected, unknown ID")
        else:
            person = person_doc['_id']
            person_name = person_doc[self.db.schema_name(id=person)]
            ...
            if "Ub-Unboarded" in person_flags:
                ...
                if person in vessel_children:
                    self.accept(who=person_name)
                    self.logger.debug(f"Person {person_name} ({person}) is acceptable")
                    person_flags.remove("Ub-Unboarded")
                    self.db.item_edit(id=person, data={"flags": person_flags})
                else:
                    self.deny(who=person_name, reason="Wrong vessel")
                    self.logger.debug(f"Person {person_name} ({person}) rejected, wrong vessel")
            else:
                self.deny(who=person_name, reason="Not unboarded")
                self.logger.debug(f"Person {person_name} ({person}) rejected, not unboarded")
